...
class OrthoLSTM(nn.Module):
    ...
    def _forward_rnn(cell, input_, length, hx):
        ...
        for time in range(max_time):
            ...
            mean_h = sum_h/(time+1)
            scalar_projection = torch.sum(h_next*mean_h, dim=1, keepdim=True) / (torch.sum(mean_h*mean_h,dim=1, keepdim=True) + eps)
            vector_component = scalar_projection* mean_h
            h_next = h_next - vector_component
            sum_h += h_next
    ...
    def forward(self, input_, length=None, hx=None):
        ...
        for layer in range(self.num_layers):
            ...
            if layer == 0:
                layer_output, layer_cell_output, (layer_h_n, layer_c_n) = OrthoLSTM._forward_rnn(
